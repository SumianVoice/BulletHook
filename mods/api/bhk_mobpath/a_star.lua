...
end bhk_mobpath.dist2 = dist2

local function distn(v, b, n)
	return math.abs(v.x - b.x)^(n or 1) + math.abs(v.y - b.y)^(n or 1) + math.abs(v.z - b.z)^(n or 1)
end bhk_mobpath.distn = distn

function bhk_mobpath.debug_particle(pos, color, time, vel, size)
	do return end
...
end bhk_mobpath.manhattan_distance = manhattan_distance
...
local function TRAVERSAL(p1, p2)
	...
	return true
end bhk_mobpath.TRAVERSAL = TRAVERSAL
...
function bhk_mobpath.smooth_path(path)
	local last_pos = path[1]
	for i, pos in ipairs(path) do
		last_pos, path[i] = path[i], (path[i] + last_pos) / 2
		bhk_mobpath.debug_particle(path[i], "#0f0", 2, nil, 2)
	end
	return path
end
...
local function collapse_path(start_node, target_node)
	...
	return bhk_mobpath.smooth_path(path)
end
...
-- You get a setmetatable version of this when using `bhk_mobpath.Options.new(options)`
bhk_mobpath.Options = {
	TRAVERSAL = TRAVERSAL,
...
function bhk_mobpath.Options.new(options)
	options = options or {}
	for k, v in pairs(bhk_mobpath.Options) do
		if options[k] == nil then options[k] = v end
	end
	return options
end

-- can be used as a template, is passed as `opt` to `bhk_mobpath.astar`
bhk_mobpath.default_options = bhk_mobpath.Options.new({
...
function bhk_mobpath.astar(start_pos, target_pos, opt)
	if not opt then opt = bhk_mobpath.default_options end
	local OPEN = {}
	local CLOSED = {}
	...
	while OPEN.__count > 0 and iteration_count < (opt.max_search or 200) do
		...
		if dist2(current.pos, target_pos) < 1 then
			bhk_mobpath.debug_particle(current.pos, "#f00", 1, UP * 100, 1)
		...
		for i, o in ipairs(opt.adjacent) do repeat
			...
			-- bhk_mobpath.debug_particle(current.pos, "#f00", 4, UP*3, 2)
			...
			if (not old_node) or (new_neighbour_node.F < old_node.F) then
				...
				bhk_mobpath.debug_particle(new_neighbour_node.pos, "#ff0", 2, nil, 1)
